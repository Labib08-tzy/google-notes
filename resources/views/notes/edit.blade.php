<x-app-layout>
    <x-slot name="header">
        <h1 class="text-xl font-semibold text-[#202124] dark:text-gray-100">Edit Note</h1>
    </x-slot>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm p-6 sm:p-8" 
             x-data="{
                ...
                handleInput() {
                    ...
                    this.autosaveStatus = 'Typing...';
                    this.debounceTimeout = setTimeout(() => {
                        this.triggerAutosave(true);
                    }, 2000);
                },

                async triggerAutosave(isDebounced = false) {
                    ...
                    if (currentTitle === this.lastSavedContent.title && currentContent === this.lastSavedContent.content) {
                        if (isDebounced) {
                    ...
                    try {
                        const response = await fetch('{{ route('notes.autosave', $note) }}', {
                            method: 'PATCH',
                            headers: {
                        ...
                        if (response.ok) {
                            ...
                            if (data.success) {
                                this.autosaveStatus = 'Saved';
                                this.lastSavedContent = { title: currentTitle, content: currentContent };
                                // Dispatch event to toast notifications
                                window.dispatchEvent(new CustomEvent('toast', { detail: { message: 'Changes autosaved', type: 'success' } }));
                            } else {
            ...
            <div class="flex items-center justify-between mb-6">
                <h2 class="text-lg font-semibold text-[#202124] dark:text-gray-100">Modify note content</h2>
                <div class="flex items-center gap-2">
                    <!-- Status dot -->
                    <span class="w-2 h-2 rounded-full transition-colors"
                          :class="{
                             'bg-gray-300': autosaveStatus === '' || autosaveStatus === 'Typing...',
                             'bg-blue-500 animate-pulse': autosaveStatus === 'Saving...',
                             'bg-green-500': autosaveStatus === 'Saved',
                             'bg-red-500': autosaveStatus === 'Save failed'
                          }"></span>
                    <span class="text-xs font-semibold uppercase tracking-wider"
                          :class="{
                             'text-gray-400': autosaveStatus === '' || autosaveStatus === 'Typing...',
                             'text-blue-500': autosaveStatus === 'Saving...',
                             'text-green-500': autosaveStatus === 'Saved',
                             'text-red-500': autosaveStatus === 'Save failed'
                          }"
                          x-text="autosaveStatus || 'Saved'"></span>
                </div>
            </div>
            ...
            <form action="{{ route('notes.update', $note) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" value="{{ old('title', $note->title) }}" @input="handleInput()"
                ...
                <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($tags as $tag)
                                <label class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#F8F9FA] dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 border border-gray-200 dark:border-gray-600 rounded-xl text-xs font-semibold text-[#202124] dark:text-gray-100 cursor-pointer transition-colors">
                                    <input type="checkbox" name="tags[]" value="{{ $tag->id }}" {{ in_array($tag->id, old('tags', $note->tags->pluck('id')->toArray())) ? 'checked' : '' }}
                    ...
                    @else
                        <p class="text-xs text-gray-400 dark:text-gray-505">
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="8" @input="handleInput()"
                              ...
                              placeholder="Write your note content here..." required>{{ old('content', $note->content) }}</textarea>
                    @error('content')
                        <p class="mt-2 text-xs text-[#EA4335]">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-50 dark:border-gray-700">
                    ...
                    <button type="submit" :disabled="autosaveStatus === 'Saving...'"
                            class="inline-flex items-center justify-center px-6 py-3 bg-[#4285F4] text-white text-sm font-medium rounded-xl hover:bg-[#3367D6] hover:shadow-lg hover:shadow-blue-200/50 transition-all duration-300 transform hover:-translate-y-0.5 disabled:opacity-55">
                        Update Note
                    </button>
                </div>
            </form>
